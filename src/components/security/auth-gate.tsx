...
export function AuthGate(): React.JSX.Element | null {
	...
	const willTryBiometric = isBiometricsEnabled && biometric.isAvailable;
	const [isBiometricPending, setIsBiometricPending] = useState(willTryBiometric);
	const biometricAttemptedRef = useRef(false);

	const shouldShow = isLocked && isRequirePin && hasPin;
	...
	useEffect(() => {
		...
		authenticateBiometric('Unlock Verso')
			.then((ok) => {
				if (ok) unlock();
				else setIsBiometricPending(false);
			})
			.catch((err: unknown) => {
				log.warn('auth-gate', 'Biometric unlock failed', err);
				setIsBiometricPending(false);
			});
	}, [shouldShow, willTryBiometric, unlock]);
	...
	return (
		<Animated.View
			...
			className="absolute inset-0 bg-background items-center justify-center gap-12"
